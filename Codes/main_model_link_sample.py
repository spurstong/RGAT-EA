...
dep_bert_output = pickle.load(open('dep_bert.pkl', "rb"))
...
DEPREL_TO_ID = {PAD_TOKEN: 0, UNK_TOKEN: 1, 'punct': 2, 'compound': 3, 'case': 4, 'nmod': 5, 'det': 6, 'nsubj': 7,
                ...
                'iobj': 38, 'det:predet': 39, 'discourse': 40, 'csubjpass': 41, 'loop': 42}


def get_dep_relation(dep):
    if dep in DEPREL_TO_ID.keys():
        return DEPREL_TO_ID[dep]
    else:
        return DEPREL_TO_ID[UNK_TOKEN]


class RGCNLayer(nn.Module):
    def __init__(self, feat_size, num_rels, out_size, activation=None, gated=False):
        ...
        self.weight = nn.Parameter(torch.Tensor(self.num_rels, self.feat_size * 2, self.out_size).float())
        self.node_weight = nn.Parameter(torch.Tensor(self.num_rels, self.feat_size, self.out_size).float())
        self.node_h = nn.Linear(self.feat_size, self.out_size, bias=False)
        self.dep_weight = nn.Parameter(torch.Tensor(self.feat_size, self.out_size).float())
        # init trainable parameters 均匀分布
        nn.init.xavier_uniform_(self.weight, gain=nn.init.calculate_gain('relu'))
        nn.init.xavier_uniform_(self.node_weight, gain=nn.init.calculate_gain('relu'))
        nn.init.xavier_uniform_(self.dep_weight, gain=nn.init.calculate_gain('relu'))
        ...
        if self.gated:
            ...
            self.attn_fc = nn.Linear(self.out_size, 1, bias=False)

    def forward(self, g, g_type="train"):

        weight = self.weight
        node_weight = self.node_weight
        # gate_weight = self.gate_weight
        attn_fc = self.attn_fc
        dep_weight = self.dep_weight

        def edge_update(edges):
            msg_dep_value = torch.matmul(edges.data['edge_rel'].unsqueeze(1), dep_weight).squeeze()
            return {"edge_rel": msg_dep_value}
        # 节点信息更新
        def message_func(edges):
            # 参数是一批边
            # 当前边类型的权重向量 （边的个数，hidden_size, 256）
            # 一句话是一个子图，训练的时候是将几个子图合成一个图，比如有三个子图，三个子图一共有7个边，则
            # 上面边的个数为7
            w = weight[edges.data['rel_type']]
            node_w = node_weight[edges.data['rel_type']]
            ...
            edge_node_two_head = torch.cat([edges.src['h'].unsqueeze(1), edges.dst['h'].unsqueeze(1)], dim=2)
            msg_edge_node = torch.bmm(edge_node_two_head, w).squeeze()
            # atten_edge = torch.cat([edges.src['h'].unsqueeze(1), edges.data['edge_value'].unsqueeze(1), edges.dst['h'].unsqueeze(1)], dim=2)
            # msg = torch.bmm(edges.src['h'].unsqueeze(1), w).squeeze()
            msg_dep_value = torch.matmul(edges.data['edge_rel'].unsqueeze(1), dep_weight).squeeze()

            # msg_dep_value = torch.bmm(edges.data['edge_rel'].unsqueeze(1), dep_weight).squeeze()
            msg = msg_edge_node * msg_dep_value
            ...
            if self.gated:
                ...
                msg = F.leaky_relu(attn_fc(msg))
                edge_node = torch.bmm(edges.src['h'].unsqueeze(1), node_w).squeeze()

            return {'z': edge_node, 'msg': msg, 'edge_rel': msg_dep_value}

        def reduce_func(nodes):

            batch_size = nodes.mailbox['msg'].shape[0]
            # 该小批次中邻居的个数
            node_degree = nodes.mailbox['msg'].shape[1]
            alpha = F.softmax(nodes.mailbox['msg'], dim=1)

            if (node_degree >= 20):
                if (node_degree <= 35):
                    top_k = math.floor(node_degree * 0.8)
                else:
                    top_k = 33
                node_edge_new, node_index = torch.sort(alpha, dim=1)
                if g_type == 'train':
                    node_index = node_index.reshape(batch_size, -1).cuda()
                    c = torch.from_numpy(np.arange(node_degree - 1, node_degree - 1 - top_k, -1)).long().cuda()
                else:
                    node_index = node_index.reshape(batch_size, -1)
                    c = torch.from_numpy(np.arange(node_degree - 1, node_degree - 1 - top_k, -1)).long()
                node_index = node_index.index_select(1, c)
                nodes_value = nodes.mailbox['z']
                edges_value = nodes.mailbox['edge_rel']
                out_info = []
                for i in range(batch_size):
                    a_in = alpha[i].index_select(0, node_index[i])
                    a_in = F.softmax(a_in, dim=0)
                    b_in = nodes_value[i].index_select(0, node_index[i])
                    edge_in = edges_value[i].ndex_select(0, node_index[i])
                    nd_v = torch.sum(a_in * b_in, dim=0)
                    ed_v = torch.sum(a_in * edge_in, dim=0)
                    c = torch.add(nd_v, ed_v)
                    out_info.append(c)
                out = torch.stack(out_info, 0)
                return {'h1': out}
            h0 = torch.sum(alpha * nodes.mailbox['edge_rel'], dim=1)
...
class RGCNModel(nn.Module):
    def __init__(self, h_dim, num_rels, num_hidden_layers=2, gated=False):
    ...
    def build_model(self):
        ...
        rgcn_layer = RGCNLayer(self.h_dim, self.num_rels, 256, activation=F.relu, gated=self.gated)
        self.layers.append(rgcn_layer)
        # rgcn_layer1 = RGCNLayer_Two(512, self.num_rels, 256, activation=F.relu, gated=self.gated)
        # self.layers.append(rgcn_layer1)

    def forward(self, g, g_type):
        rst_out = []
...
class Head(nn.Module):
    ...
    def __init__(self, gcn_out_size: int, bert_out_size: int):
        ...
        self.fc = nn.Sequential(
            ...
            nn.Dropout(0.5),
            nn.Linear(bert_out_size * 3 + gcn_out_size * 3, 512),
            nn.ReLU(),
            nn.BatchNorm1d(512),
            nn.Dropout(0.5),
            # 结果是长度为6的一维数组
...
class GPRModel(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.RGCN = RGCNModel(h_dim=1024, num_rels=3, gated=False)
...
for i, sent_token in enumerate(token_lst):
    ...
    sent_token = sent_token[1:cu_offsets[0]] + sent_token[cu_offsets[0] + 1:cu_offsets[1]] + sent_token[cu_offsets[1] + 1: cu_offsets[4]] + sent_token[cu_offsets[4] + 1: cu_offsets[2]] + sent_token[cu_offsets[2] + 1: cu_offsets[3]] + sent_token[cu_offsets[3] + 1:-1]  # 将句子中的开始标签和结尾标签去除掉 sent_token[1:-1]
    ...
    edge_type = []
    edge_relation = []
    parse_token_lst = parse_rst['tokens']

    extra_sencond_offset_list = []
    extra_third_offset_list = []
    for i_word, word in enumerate(parse_token_lst):
        if is_target(word['head'], target_offset_list):
            extra_sencond_offset_list.append(word['id'])
    
    target_offset_list = target_offset_list + extra_sencond_offset_list
    
    for i_word, word in enumerate(parse_token_lst):
        if is_target(word['head'], extra_sencond_offset_list) and not is_target(i_word, target_offset_list):
            extra_third_offset_list.append(i_word)
    target_offset_list = target_offset_list + extra_third_offset_list
    ...
    for i_word, word in enumerate(parse_token_lst):
        ...
        if i_word not in nodes:
            ...
            edge_type.append(0)
            edge_relation.append(dep_bert_output[DEPREL_TO_ID['loop']])
        ...
        if not ([word['head'], word['id']] in edges):
            ...
            dep_rel = parse_token_lst[word['head']]['dep']

            dep_rel_id = get_dep_relation(dep_rel)
            edge_relation.append(dep_bert_output[dep_rel_id])
            edge_type.append(1)
        if not ([word['id'], word['head']] in edges):
            edges.append([word['id'], word['head']])
            dep_rel_id = get_dep_relation(word['dep'])
            edge_type.append(2)
            dep_rel_id = get_dep_relation(word['dep'])
            edge_relation.append(dep_bert_output[dep_rel_id])
    ...
    # no_event_trigger_nodes = []
    # event_trigger_nodes = event1_nodes + event2_nodes
    # for node in range(num_nodes):
    #     if (node not in event_trigger_nodes):
    #         no_event_trigger_nodes.append(node)
    # old_edges = tran_edges
    # for index1 in range(0, len(no_event_trigger_nodes)-1):
    #     index1_node = no_event_trigger_nodes[index1]
    #     for index2 in range(index1+1, len(no_event_trigger_nodes)):
    #         index2_node = no_event_trigger_nodes[index2]
    #         is_right = 1
    #         for index, edge in enumerate(old_edges):
    #             if ((index1_node == edge[0] and index2_node == edge[1]) or (index1_node == edge[1] and index2_node == edge[0])):
    #                 is_right = 0
    #                 break;
    #         if (is_right == 1):
    #             tran_edges.append([index1_node, index2_node])
    #             edge_type.append(3)
    #             edge_relation.append(dep_bert_output[DEPREL_TO_ID[UNK_TOKEN]])
    #             tran_edges.append([index2_node, index1_node])
    #             edge_type.append(3)
    #             edge_relation.append(dep_bert_output[DEPREL_TO_ID[UNK_TOKEN]])
    ...
    edge_relation = torch.stack(edge_relation, dim=0).to(device)
    G.edata.update({'rel_type': edge_type, })
    G.edata.update({'norm': edge_norm})
    G.edata.update({'edge_rel': edge_relation})
    all_graphs.append(G)
...
for i, sent_token in enumerate(test_token_lst):
    ...
    sent_token = sent_token[1:cu_test_offsets[0]] + sent_token[cu_test_offsets[0] + 1:cu_test_offsets[1]] + sent_token[cu_test_offsets[1] + 1: cu_test_offsets[4]] + sent_token[cu_test_offsets[4] + 1: cu_test_offsets[2]] + sent_token[cu_test_offsets[2] + 1: cu_test_offsets[3]] + sent_token[cu_test_offsets[3] + 1:-1]  # 将句子中的开始标签和结尾标签去除掉 sent_token[1:-1]
    # bert可以会把一个词分成多个词，分的词前部添加#，所以要去掉#
    ...
    edge_type = []
    edge_relation = []
    paese_token_lst = parse_rst['tokens']

    extra_sencond_offset_list = []
    extra_third_offset_list = []
    for i_word, word in enumerate(paese_token_lst):
        if is_target(word['head'], target_offset_list):
            extra_sencond_offset_list.append(word['id'])
    ...
    for i_word, word in enumerate(paese_token_lst):
        ...
        if not ([word['head'], word['id']] in edges):
            edges.append([word['head'], word['id']])
            dep_rel = paese_token_lst[word['head']]['dep']
    ...
    #         is_right = 1
    #         for index, edge in enumerate(old_edges):
    #             if ((index1_node == edge[0] and index2_node == edge[1]) or (
    #                     index1_node == edge[1] and index2_node == edge[0])):
    #                 is_right = 0
    #                 break;
    #         if (is_right == 1):
    #             tran_edges.append([index1_node, index2_node])
...
for train_index, test_index in kfold.split(train_df, train_y):
    ...
    for epoch in range(total_epoch):
        ...
        m_p = precision_score(val_labels,val_predictions, labels=[0, 1, 3, 4, 5], average='micro')
        m_r = recall_score(val_labels, val_predictions, labels=[0, 1, 3, 4, 5], average='micro')
        ...
        print('验证集 Epoch {}, micro-准确率值 {}'.format(epoch, m_p))
        print('验证集 Epoch {}, micro-召回率值 {}'.format(epoch, m_r))
        ...
        if val_loss < best_val_loss:
            ...
            if epoch > 20:
                torch.save(model.state_dict(), 'best_model_gpu_type.pth')
    ...
    model = GPRModel()
    model.load_state_dict(torch.load('best_model_gpu_type.pth', map_location=torch.device('cpu')))
    model.eval()
    test_labels = []
    # all_relation = ['a', 'b', 's', 'i', 'ii', 'v']
