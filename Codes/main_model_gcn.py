...
class RGCNLayer(nn.Module):
    def __init__(self, feat_size, out_size, activation=None, gated=True):
        ...
        self.linear = nn.Linear(self.feat_size, out_size)
    ...
    def forward(self, g):
        ...
        def deal_node(node):
            h = linear(node.data['h'])
            h = activ(h)
            return {"h": h}

        g.apply_nodes(deal_node)
...
class RGCNModel(nn.Module):
    ...
    def build_model(self):
        ...
        rgcn_layer = RGCNLayer(self.h_dim, 512, activation=F.relu, gated=self.gated)
        self.layers.append(rgcn_layer)
        rgcn_layer1 = RGCNLayer(512, 256, activation=F.relu, gated=self.gated)
...
class GPRModel(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.RGCN = RGCNModel(h_dim=1024, num_rels=4, gated=False)
...
for i, sent_token in enumerate(token_lst):
    ...
    sent_token = sent_token[1:cu_offsets[0]] + sent_token[cu_offsets[0] + 1:cu_offsets[1]] + sent_token[
                                                                                             cu_offsets[1] + 1:
                                                                                             cu_offsets[
                                                                                                 4]] + sent_token[
                                                                                                       cu_offsets[
                                                                                                       ...
                                                                                                       cu_offsets[
                                                                                                           2]] + sent_token[
                                                                                                                 ...
                                                                                                                 cu_offsets[
                                                                                                                     3]] + sent_token[
                                                                                                                           cu_offsets[
                                                                                                                               3] + 1:-1]  # 将句子中的开始标签和结尾标签去除掉 sent_token[1:-1]
...
for i, sent_token in enumerate(test_token_lst):
    ...
    sent_token = sent_token[1:cu_test_offsets[0]] + sent_token[cu_test_offsets[0] + 1:cu_test_offsets[1]] + sent_token[
                                                                                                            cu_test_offsets[
                                                                                                                1] + 1:
                                                                                                            cu_test_offsets[
                                                                                                                4]] + sent_token[
...
for train_index, test_index in kfold.split(train_df, train_y):
    ...
    for epoch in range(total_epoch):
        ...
        m_f = f1_score(val_labels, val_predictions, labels=[0, 1], average='micro')
